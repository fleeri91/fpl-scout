...
export function DashboardScreen({
  ...
  const squadNote =
    injuredCount === 0 && riskCount === 0
      ...
      : [
          injuredCount
            ? `${injuredCount} ${injuredCount > 1 ? 'injuries' : 'injury'}`
            : null,
  ...
  return (
    <div className="flex flex-col gap-4.5 p-5.5">
      <div className="grid grid-cols-4 gap-3.5">
        {summary.map((s) => (
          <Card key={s.label} className="border-border gap-1 rounded-xl p-4">
      ...
      <div className="grid grid-cols-[minmax(0,2.1fr)_minmax(0,1fr)] items-start gap-4.5">
        <Card className="border-border gap-0 rounded-xl py-0">
          ...
          <CardContent className="py-3.5">
            <div className="mb-2.5 text-[11px] tracking-wider text-(--fg3) uppercase">
              Starting XI
            </div>
        ...
        <Card className="border-border gap-0 rounded-xl py-0">
          ...
          <CardContent className="flex flex-col gap-2.25 py-3">
            ...
            {alerts.length === 0 ? (
              <div className="p-4.5 text-center text-xs text-(--fg3)">
